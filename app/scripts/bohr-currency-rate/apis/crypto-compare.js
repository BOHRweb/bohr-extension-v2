...
function getPricingURL(currentCurrency, nativeCurrency, includeUSDRate) {
  ...
  return "https://api.hotcoinfin.com/v1/market/ticker"
}
/**
...
function fetchExchangeRate(currency, nativeCurrency, includeUSDRate) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const jsonArr = yield util_1.handleFetch(getPricingURL(currency, nativeCurrency, includeUSDRate));
      let usdPricie = 0;
      for(const item of jsonArr.ticker){
        if(item.symbol == "br_usdt"){
          usdPricie = item.last;
          break;
        }
      }
      const conversionRate = Number(usdPricie);
      const usdConversionRate = Number(usdPricie);

      if (!Number.isFinite(conversionRate)) {
            throw new Error(`Invalid response for ${currency.toUpperCase()}: ${usdPricie}`);
        }
        if (includeUSDRate && !Number.isFinite(usdConversionRate)) {
            throw new Error(`Invalid response for usdConversionRate: ${usdPricie}`);
        }
        return {
