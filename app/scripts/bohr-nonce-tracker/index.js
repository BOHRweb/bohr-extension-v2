...
class NonceTracker {
  ...
  */
  ...
  async _getNetworkNextNonce (address) {
    ...
    const blockNumberResult = await api.blockNumber({});
    const blockNumber = '0x'+Number(blockNumberResult.result).toString(16)

    const dataResult = await api.account({address:address});
    const baseCount = parseInt(dataResult.result.nonce) ;
