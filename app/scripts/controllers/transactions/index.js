...
import NonceTracker from '../../bohr-nonce-tracker/index';
...
export default class TransactionController extends EventEmitter {
  ...
  */
   ...
   * new transaction contains the same nonce as the previous, is a basic BR transfer of 0x value to
  ...
  */
  async signTransaction(txId) {
    ...
    const accountBohr = await api.account({ address:fromAddress});
    ethTx.nonce = ethUtil.toBuffer(Number(accountBohr.result.nonce))

    const ethTxNew = await this.signEthTx(ethTx, fromAddress);
  ...
  */
  async publishTransaction(txId, rawTx) {
    ...
    try {
      ...
      const txResult = await api.broadcastRawTransaction({ raw:rawTx});
