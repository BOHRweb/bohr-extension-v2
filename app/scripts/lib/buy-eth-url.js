 ...
 * @param {string} opts.address - The address the bought BR should be sent to.  Only relevant if chainId === '0x1'.
 * @returns {string|undefined} The url at which the user can access BR, while in the given chain. If the passed
...
export default function getBuyEthUrl({ chainId, address, service }) {
  ...
  switch (service) {
    case 'wyre':
      return `https://pay.sendwyre.com/purchase?dest=ethereum:${address}&destCurrency=BR&accountId=AC-7AG3W4XH4N2&paymentMethod=debit-card`;
