 ...
 * 5.  Convert transactions bohrNetworkId to decimal if they are hex
...
function transformState(state = {}) {
  ...
  if (Array.isArray(transactions)) {
    transactions.forEach((transaction) => {
      const bohrNetworkId = transaction?.bohrNetworkId;
      if (
        typeof bohrNetworkId === 'string' &&
        hexRegEx.test(bohrNetworkId)
      ) {
        transaction.bohrNetworkId = parseInt(
          bohrNetworkId,
          16,
        ).toString(10);
      }
