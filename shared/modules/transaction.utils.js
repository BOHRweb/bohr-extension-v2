...
export function transactionMatchesNetwork(transaction, chainId, networkId) {
  ...
  return transaction.bohrNetworkId === networkId;
}

/**
 * build the etherscan link for a transaction by either chainId, if available
 * or bohrNetworkId as a fallback. If rpcPrefs is provided will build the
 * url for the provided blockExplorerUrl.
 *
 * @param {Object} transaction - a transaction object from state
 * @param {string} [transaction.bohrNetworkId] - network id tx occurred on
...
export function getBlockExplorerUrlForTx(transaction, rpcPrefs = {}) {
  ...
  // }
  // return createExplorerLink(transaction.hash, transaction.bohrNetworkId);
  return "https://explorer.bohrweb.org/#/block-chain/txns-detail?hash="+ transaction.hash
}
