...
function RecipientWithAddress({
  ...
  if (recipientName !== undefined){
    recipientName = recipientName.indexOf("0x") != -1 ? shortenAddress(checksummedRecipientAddress) : recipientName;
  }
  return (
    <div
    ...
    >
      {!addressOnly && (
        <div className="sender-to-recipient__sender-icon">
          <Identicon
            address={checksumAddress(recipientAddress)}
            diameter={24}
            image={assetImage}
          />
        </div>
...
export default function SenderToRecipient({
  ...
  const checksummedSenderAddress = addressEncode(checksumAddress(senderAddress));
  const checksummedRecipientAddress = addressEncode(checksumAddress(recipientAddress));
