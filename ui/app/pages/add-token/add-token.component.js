...
import abiindex from '../../../../app/scripts/lib/abi-index';
...
class AddToken extends Component {
  ...
  handleNext() {
    ...
    const customToken = {
      address: addressDecode16(address),
      symbol,
      decimals,
    };
  ...
  async attemptToAutoFillTokenParams(address) {
    ...
    let decimals = 0;
    try {
      const dataSymbol = await api.walletLocalCall({
        to: address,
        data: web3Utils.sha3('symbol()').slice(0, 10).replace("0x", "")
      });

      if (dataSymbol.success && dataSymbol.result) {
        symbol = abiindex.decodeParameter("string", dataSymbol.result.returnData)
      }
    } catch (e) {
    ...
    try {
      const dataDecimals = await api.walletLocalCall({
        to: address,
        data: web3Utils.sha3('decimals()').slice(0, 10).replace("0x", "")
      });

      if (dataDecimals.success && dataDecimals.result) {
        decimals = abiindex.decodeParameter("uint256", dataDecimals.result.returnData)
      }
    } catch (e) {
  ...
  handleCustomAddressChange(value) {
    ...
    if (customAddress.indexOf('B') !== 0 || customAddress.length !== 35) {
      this.setState({
        customAddress,
    ...
    this.setState({
      customAddress: addressEncode(customAddress),
      customAddressError: null,
      tokenSelectorError: null,
      autoFilled: false,
    });

    customAddress = addressDecode16(customAddress);

    const isValidAddress = ethUtil.isValidAddress(customAddress);
    const standardAddress = addHexPrefix(customAddress);
