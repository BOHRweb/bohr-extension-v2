...
export default class EnsInput extends Component {
  ...
  onChange = (e) => {
    ...
    this.setState({ input }, () => onChange(inputTemp));

    if (input.length !== 35 || input.charAt(0) !== 'B') {
      return null;
    }
    // must be encoded for next step
    input = addressDecode16(input);
    console.log('onChange-----input----decoded', input);
        ...
        <div className="ens-input__wrapper ens-input__wrapper--valid">
          ...
          >
            <div className="ens-input__selected-input__title">
              {name || ellipsify(addressEncode(selectedAddress))}
            </div>
            {name && (
              <div className="ens-input__selected-input__subtitle">
                {addressEncode(selectedAddress)}
              </div>
            )}
          </div>
          <div
