...
import { addressDecode16 } from '../../../../../mixins/mixin';
...
export default class AddContact extends PureComponent {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (nextProps.qrCodeData) {
      if (nextProps.qrCodeData.type === 'address') {
        const scannedAddress = addressDecode16(nextProps.qrCodeData.values.address.toLowerCase());
        const currentAddress = addressDecode16(this.state.ensAddress) || addressDecode16(this.state.ethAddress);
  ...
  validate = (address) => {
    ...
    // }
    if (address.length === 35 && address.charAt(0) === 'B') {
      this.setState({ error: '', ethAddress: address });
    } else {
      this.setState({ error: 'Invalid Address' });
    }
        ...
        <PageContainerFooter
          ...
          onSubmit={async () => {
            await addToAddressBook(
              addressDecode16(this.state.ethAddress),
              this.state.newName,
            );
