...
export default class NetworkForm extends PureComponent {
  ...
  setStateWithValue = (stateKey, validator) => {
    return (e) => {
      console.log('validateUrlRpcUrl-----rpcUrl-----onChange', e.target.value);
  ...
  validateChainIdOnSubmit = async (formChainId, parsedChainId, rpcUrl) => {
    ...
    try {
      ...
      console.log('jsonRpcRequest------err', err);
  ...
  validateUrlRpcUrl = (url, stateKey) => {
    ...
    console.log('validateUrlRpcUrl-----rpcUrl', rpcUrl);

    // const data = await api.checkApiUrl({apiUrl:rpcUrl});
    //
    // if (data && data.success) {
    //   this.setErrorTo(stateKey, '');
    // }else {
    //   this.setErrorTo(stateKey, this.context.t('invalidRPC'));
    // }
    ...
    } else if(rpcUrl.indexOf("v2.4.0/") == -1){
      this.setErrorTo(stateKey, this.context.t('invalidRPC'));
