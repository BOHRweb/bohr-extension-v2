...
export function getBoHrIdentities(state) {
...
export function accountsWithSendEtherInfoSelector(state) {
  const accounts = getMetaMaskAccounts(state);
  const identities = getBoHrIdentities(state);
 ...
 * @property {string} symbol - The symbol for BR, namely "BR"
 * @property {string} name - The name of the BR currency, "Ether"
 * @property {string} address - A substitute address for the metaswap-api to
 * recognize the BR token
 * @property {string} decimals - The number of BR decimals, i.e. 18
 * @property {string} balance - The user's BR balance in decimal wei, with a
 * precision of 4 decimal places
 * @property {string} string - The user's BR balance in decimal BR
 */
 ...
 * selected account's BR balance, as expected by the Swaps API.
 */
