...
export function updateSendTokenBalance({ sendToken, tokenContract, address }) {
  return (dispatch) => {
    ...
    const paramObj = {
      to:tokenContract ? tokenContract.address :"",
      data:funStr
    }
    let result = {"0":"0","balance":"0"};

    const tokenBalancePromise = tokenContract
      ? api.walletLocalCall(paramObj)
      : Promise.resolve();
    return tokenBalancePromise
      .then((usersToken) => {
        if (usersToken) {

          const ba = new BN(ethUtil.stripHexPrefix(usersToken.result.returnData), 16)

          let resultToken = {"0":"0","balance":"0"};
          resultToken[0] = ba;
          resultToken.balance = ba;

          const newTokenBalance = calcTokenBalance({ sendToken, usersToken:resultToken });
...
export function setRpcTarget(newRpc, chainId, ticker = 'BR', nickname) {
  return async (dispatch) => {
    log.debug(
...
export function getOpenBoHrTabsIds() {
  return async (dispatch) => {
    const openMetaMaskTabIDs = await promisifiedBackground.getOpenBoHrTabsIds();
